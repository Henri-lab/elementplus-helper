...
import {
  ...
  changeTreeList,
  getDetailsList,
} from "@/api/behindInterface/tree/index";
...
function handleAdd(row) {
  ...
  getTreeList().then((response) => {
    ...
    form.value.parentId = row.id ? row.id : 0;
  });
  if (row != undefined) {
...
function submitForm() {
  ...
  if (form.value.id != undefined) {
    ...
    changeTreeList(form.value).then((response) => {
      proxy.$modal.msgSuccess("修改成功");
      open.value = false;
      getList();
    });
  } else {
    addTreeList(form.value).then((response) => {
...
function handleDelete(row) {
  proxy.$modal
    .confirm("是否确认删除?")
    .then(function () {
      return removeTargetList(row.id);
    })
    .then(() => {
      getList();
