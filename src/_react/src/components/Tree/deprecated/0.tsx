...
const TreeWithContextMenu: React.FC<TreeWithContextMenuProps> = ({
  ...
  const addNode = (nodes: TreeNode[], parentKey: string, newNode: TreeNode) => {
    setData(
      nodes.map(node => {
  ...
  const deleteNode = (nodes: TreeNode[], key: string) => {
    const updatedData = nodes.filter(node => node.key !== key).map(node => {
      if (node.children) {
  ...
  const updateNode = (nodes: TreeNode[], key: string, newTitle: string) => {
  ...
  const handleEditSubmit = () => {
    if (selectedNode) {
      updateNode(data, selectedNode.key, editingText);
    }
    setIsEditing(false);
  };
